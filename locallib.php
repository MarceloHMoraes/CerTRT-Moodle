...
/**
 * CerTRT module internal API,
 * this is in separate file to reduce memory use on non-certrt pages.
 *
 * @package    mod_certrt
...
/** Set CERT_PER_PAGE to 0 if you wish to display all certrts on the report page */
 ...
 */
function certrt_get_teachers($certrt, $user, $course, $cm) {
    global $USER;

    $context = context_module::instance($cm->id);
    $potteachers = get_users_by_capability($context, 'mod/certrt:manage',
        '', '', '', '', '', '', false, false);
...
/**
 * Alerts teachers by email of received certrts. First checks
 * whether the option to email teachers is set for this certrt.
 *
 * @param stdClass $course
 * @param stdClass $certrt
 ...
 */
function certrt_email_teachers($course, $certrt, $certrecord, $cm) {
    global $USER, $CFG, $DB;
...
/**
 * Alerts others by email of received certrts. First checks
 * whether the option to email others is set for this certrt.
 ...
 * @param stdClass $cm course module
 */
function certrt_email_others($course, $certrt, $certrecord, $cm) {
    global $USER, $CFG;
 ...
 * @return string
 */
function certrt_email_teachers_html($info) {
    $posthtml  = '<font face="sans-serif">';
    $posthtml .= '<p>' . get_string('emailteachermailhtml', 'certrt', $info) . '</p>';
    $posthtml .= '</font>';
...
/**
 * Sends the student their issued certrt from moddata as an email
 * attachment.
 *
 * @param stdClass $course
 * @param stdClass $certrt
 ...
 */
function certrt_email_student($course, $certrt, $certrecord, $context, $filecontents, $filename) {
    global $USER;
 ...
 * @param int $certrecordid the certrt issue record id
 * @param string $filename pdf filename
 * @param int $contextid context id
 * @return bool return true if successful, false otherwise
 */
function certrt_save_pdf($pdf, $certrecordid, $filename, $contextid) {
    global $USER;
    ...
    // luckily there is supposed to be always only one certrt here.
...
/**
 * Produces a list of links to the issued certrts.  Used for report.
 *
 * @param stdClass $certrt
 * @param int $userid
 * @param int $contextid
 ...
 */
function certrt_print_user_files($certrt, $userid, $contextid) {
    global $CFG, $DB, $OUTPUT;

    $output = '';

    $certrecord = $DB->get_record('certrt_issues', array('userid' => $userid, 'certrtid' => $certrt->id));
    $fs = get_file_storage();
    ...
    foreach ($files as $file) {
        ...
        $link = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$contextid.'/mod_certrt/issue/'.$certrecord->id.'/'.$filename);
...
/**
 * Inserts preliminary user data when a certrt is viewed.
 * Prevents form from issuing a certrt upon browser refresh.
 *
 * @param stdClass $course
 * @param stdClass $user
 * @param stdClass $certrt
 * @param stdClass $cm
 * @return stdClass the newly created certrt issue
 */
function certrt_get_issue($course, $user, $certrt, $cm) {
    global $DB;

    // Check if there is an issue already, should only ever be one
    if ($certissue = $DB->get_record('certrt_issues', array('userid' => $user->id, 'certrtid' => $certrt->id))) {
        return $certissue;
    }

    // Create new certrt issue record
    $certissue = new stdClass();
    $certissue->certrtid = $certrt->id;
    $certissue->userid = $user->id;
    $certissue->code = certrt_generate_code();
    $certissue->timecreated =  time();
    $certissue->id = $DB->insert_record('certrt_issues', $certissue);

    // Email to the teachers and anyone else
    certrt_email_teachers($course, $certrt, $certissue, $cm);
    certrt_email_others($course, $certrt, $certissue, $cm);

    return $certissue;
}

/**
 * Returns a list of issued certrts - sorted for report.
 *
 * @param int $certrtid
 ...
 */
function certrt_get_issues($certrtid, $sort="ci.timecreated ASC", $groupmode, $cm, $page = 0, $perpage = 0) {
    global $DB, $USER;
    ...
    // Get all users that can manage this certrt to exclude them from the report.
    $certmanagers = array_keys(get_users_by_capability($context, 'mod/certrt:manage', 'u.id'));
    ...
    // Get all the users that have certrts issued, should only be one issue per user for a certrt
    $allparams = $conditionsparams + array('certrtid' => $certrtid);
    ...
    $users = $DB->get_records_sql("SELECT $picturefields, u.idnumber, ci.code, ci.timecreated
                                     FROM {user} u
                               INNER JOIN {certrt_issues} ci
                                       ON u.id = ci.userid
                                    WHERE u.deleted = 0
                                      AND ci.certrtid = :certrtid $conditionssql
...
/**
 * Returns a list of previously issued certrts--used for reissue.
 *
 * @param int $certrtid
 * @return stdClass the attempts else false if none found
 */
function certrt_get_attempts($certrtid) {
    global $DB, $USER;

    $sql = "SELECT *
              FROM {certrt_issues} i
             WHERE certrtid = :certrtid
               AND userid = :userid";
    if ($issues = $DB->get_records_sql($sql, array('certrtid' => $certrtid, 'userid' => $USER->id))) {
        return $issues;
    }
...
/**
 * Prints a table of previously issued certrts--used for reissue.
 *
 * @param stdClass $course
 * @param stdClass $certrt
 ...
 */
function certrt_print_attempts($course, $certrt, $attempts) {
    global $OUTPUT;

    echo $OUTPUT->heading(get_string('summaryofattempts', 'certrt'));

    // Prepare table header
    $table = new html_table();
    $table->class = 'generaltable';
    $table->head = array(get_string('issued', 'certrt'));
    $table->align = array('left');
    $table->attributes = array("style" => "width:20%; margin:auto");
    $gradecolumn = $certrt->printgrade;
    if ($gradecolumn) {
    ...
    foreach ($attempts as $attempt) {
        ...
        if ($gradecolumn) {
            $attemptgrade = certrt_get_grade($certrt, $course);
            $row[] = $attemptgrade;
        }
 ...
 */
function certrt_get_course_time($courseid) {
    global $CFG, $DB, $USER;
 ...
 * @return array
 */
function certrt_get_grade_options() {
    $gradeoptions['0'] = get_string('no');
    $gradeoptions['1'] = get_string('coursegrade', 'certrt');
 ...
 * @return array
 */
function certrt_get_date_options() {
    $dateoptions['0'] = get_string('no');
    $dateoptions['1'] = get_string('issueddate', 'certrt');
    $dateoptions['2'] = get_string('completiondate', 'certrt');
 ...
 * @return array
 */
function certrt_get_grade_categories($courseid) {
...
function certrt_get_outcomes() {
    ...
    if (isset($printoutcome)) {
        ...
        $outcomeoptions['0'] = get_string('nooutcomes', 'certrt');
    }
...
/**
 * Get certrt types indexed and sorted by name for mod_form.
 *
 * @return array containing the certrt type
 */
function certrt_types() {
    $types = array();
    $names = get_list_of_plugins('mod/certrt/type');
    $sm = get_string_manager();
    foreach ($names as $name) {
        if ($sm->string_exists('type'.$name, 'certrt')) {
            $types[$name] = get_string('type'.$name, 'certrt');
        } else {
 ...
 * @return array
 */
function certrt_get_images($type) {
    global $CFG;

    switch($type) {
        case CERT_IMAGE_BORDER :
            $path = "$CFG->dirroot/mod/certrt/pix/borders";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/borders";
            break;
        case CERT_IMAGE_SEAL :
            $path = "$CFG->dirroot/mod/certrt/pix/seals";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/seals";
            break;
        case CERT_IMAGE_SIGNATURE :
            $path = "$CFG->dirroot/mod/certrt/pix/signatures";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/signatures";
            break;
        case CERT_IMAGE_WATERMARK :
            $path = "$CFG->dirroot/mod/certrt/pix/watermarks";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/watermarks";
            break;
    }
    // If valid path
    if (!empty($path)) {
        $options = array();
        $options += certrt_scan_image_dir($path);
        $options += certrt_scan_image_dir($uploadpath);
 ...
 */
function certrt_get_mod_grade($course, $moduleid, $userid) {
    global $DB;
...
/**
 * Returns the date to display for the certrt.
 *
 * @param stdClass $certrt
 * @param stdClass $certrecord
 * @param stdClass $course
 ...
 */
function certrt_get_date($certrt, $certrecord, $course, $userid = null) {
    global $DB, $USER;
    ...
    if ($certrt->printdate == '2') {
        ...
        if ($modinfo = certrt_get_mod_grade($course, $certrt->printdate, $userid)) {
            $date = $modinfo->dategraded;
        }
    }

    //mhz - portuguese format date
    if ($certrt->printdate > 0) {
        if ($certrt->datefmt == 1) {
            $certrtdate = userdate($date, '%B %d, %Y');
        } else if ($certrt->datefmt == 2) {
            $suffix = certrt_get_ordinal_number_suffix(userdate($date, '%d'));
            $certrtdate = userdate($date, '%B %d' . $suffix . ', %Y');
        } else if ($certrt->datefmt == 3) {
            $certrtdate = userdate($date, '%d de %B de %Y.'); //mhz - correct phrase in Portuguese
        } else if ($certrt->datefmt == 4) {
            $certrtdate = userdate($date, '%B %Y');
        } else if ($certrt->datefmt == 5) {
            $certrtdate = userdate($date, get_string('strftimedate', 'langconfig'));
        }
 ...
 */
function certrt_get_ordinal_number_suffix($day) {
...
/**
 * Returns the grade to display for the certrt.
 *
 * @param stdClass $certrt
 * @param stdClass $course
 * @param int $userid
 ...
 */
function certrt_get_grade($certrt, $course, $userid = null, $valueonly = false) {
    global $USER;
    ...
    if ($certrt->printgrade > 0) {
        if ($certrt->printgrade == 1) {
            ...
            if ($modinfo = certrt_get_mod_grade($course, $certrt->printgrade, $userid)) {
                // Check we want to add a prefix to the grade.
                $strprefix = '';
                if (!$valueonly) {
                    $strprefix = $modinfo->name . ' ' . get_string('grade', 'certrt') . ': ';
                }
                if ($certrt->gradefmt == 1) {
    ...
    } else if ($certrt->printgrade < 0) { // Must be a category id.
        if ($category_item = grade_item::fetch(array('itemtype' => 'category', 'iteminstance' => -$certrt->printgrade))) {
...
/**
 * Returns the outcome to display on the certrt
 *
 * @param stdClass $certrt
 * @param stdClass $course
 * @return string the outcome
 */
function certrt_get_outcome($certrt, $course) {
    global $USER;

    if ($certrt->printoutcome > 0) {
        if ($grade_item = new grade_item(array('id' => $certrt->printoutcome))) {
...
/**
 * Returns the code to display on the certrt.
 *
 * @param stdClass $certrt
 * @param stdClass $certrecord
 ...
 */
function certrt_print_text($pdf, $x, $y, $align, $font='freeserif', $style, $size = 10, $text, $width = 0) {
 ...
 * @param stdClass $certrt
 */
function certrt_draw_frame($pdf, $certrt) {
    if ($certrt->bordercolor > 0) {
        if ($certrt->bordercolor == 1) {
            $color = array(0, 0, 0); // black
        }
        if ($certrt->bordercolor == 2) {
            $color = array(153, 102, 51); // brown
        }
        if ($certrt->bordercolor == 3) {
            $color = array(0, 51, 204); // blue
        }
        if ($certrt->bordercolor == 4) {
            $color = array(0, 180, 0); // green
        }
        switch ($certrt->orientation) {
            case 'L':
 ...
 * @param stdClass $certrt
 */
function certrt_draw_frame_letter($pdf, $certrt) {
    if ($certrt->bordercolor > 0) {
        if ($certrt->bordercolor == 1)    {
            $color = array(0, 0, 0); //black
        }
 ...
 */
function certrt_print_image($pdf, $certrt, $type, $x, $y, $w, $h) {
    global $CFG;

    switch($type) {
        case CERT_IMAGE_BORDER :
            $attr = 'borderstyle';
            $path = "$CFG->dirroot/mod/certrt/pix/$type/$certrt->borderstyle";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/$type/$certrt->borderstyle";
            break;
        case CERT_IMAGE_SEAL :
            $attr = 'printseal';
            $path = "$CFG->dirroot/mod/certrt/pix/$type/$certrt->printseal";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/$type/$certrt->printseal";
            break;
        case CERT_IMAGE_SIGNATURE :
            $attr = 'printsignature';
            $path = "$CFG->dirroot/mod/certrt/pix/$type/$certrt->printsignature";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/$type/$certrt->printsignature";
            break;
        case CERT_IMAGE_WATERMARK :
            $attr = 'printwmark';
            $path = "$CFG->dirroot/mod/certrt/pix/$type/$certrt->printwmark";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/$type/$certrt->printwmark";
            break;
    }
    // Has to be valid
    if (!empty($path)) {
 ...
 * @return string
 */
function certrt_generate_code() {
    global $DB;

    $uniquecodefound = false;
    $code = random_string(10);
    while (!$uniquecodefound) {
        if (!$DB->record_exists('certrt_issues', array('code' => $code))) {
 ...
 * @return array
 */
function certrt_scan_image_dir($path) {
...
/**
 * Get normalised certrt file name without file extension.
 *
 * @param stdClass $certrt
 * @param stdClass $cm
 * @param stdClass $course
 ...
 */
function certrt_get_certrt_filename($certrt, $cm, $course) {
    $coursecontext = context_course::instance($course->id);
    $coursename = format_string($course->shortname, true, array('context' => $coursecontext));

    $context = context_module::instance($cm->id);
    $name = format_string($certrt->name, true, array('context' => $context));
   ...
   /* VMF */
    /**
     * Create the default information for the verse page of the certificate.
     *
     * @param Course Id
     * @return string html for pdf renderization
     */
    function defaultVerso($course_id) {
        Global $DB;
        $curso = $DB->get_record('course', array('id' => $course_id));
        $nomeCurso = strtoupper($curso->fullname);
        $texto = "<h1 style=\"text-align:center;\"> $nomeCurso </h1> "
                . "<br/><br/>"
                . "<p> </p><h2 style=\"text-align:center;\">CONTEUDISTAS</h2> "
                . "<p style=\"text-align:center;\">...</p>"
                . "<p> </p><h2 style=\"text-align:center;\">TUTORES</h2>     "
                . "<p style=\"text-align:center;\">...</p>";
        /* $default_2ndPage = "<h1  style="text-align: center;'> $curso->fullname </h1><br/><h2 style='text-align: center;'>CONTEUDISTAS</h2><ul>" ;

          /* WHILE
          $default_2ndPage .= "<li style='text-align: left; margin-left:40px;'><p> </p></li>" ;

          $default_2ndPage .= "</ul><br/><h2 style='text-align: center;'>TUTORES</h2><ul>";
        ...
        //$texto = "<h2 syle='text-align:center;'>M&Oacute;DULOS</h2><ul style='text-align:center;' >";//$this->current->course;
        $texto .= "<p> </p><h2 style=\"text-align:center;\">M&Oacute;DULOS</h2><div style=\"text-align:center;\">";
        foreach ($result as $record) {
            if ($record->name){
                $texto .= "<p>$record->name</p>"; //$texto .= "<li> $record->name </li>";
            }
        }        
        return $texto;
    }
